...
export async function llmPromptPreparation(userQuestion: string,
    ...
    if (editorMode) {
        ...
        mainPrompt.template += `{userQuestion} \n I expect the response directly in the corresponding VALID Langium textual syntax according to the grammar provided, without any markdown and/or backticks.`;
...
export async function llmFetchResponse(
    ...
    while (retries <= MAX_RETRIES) {
        ...
        try {
            const document = await parse(rawResponse.content as string);
...
function extractValidationErrorMessages(validationErrors: Diagnostic[]): string[] {
    return validationErrors.map(error => {
        const position = error.range
            ? ` at line ${error.range.start.line + 1}, column ${error.range.start.character + 1}`
            : '';
